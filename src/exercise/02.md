...
- React (creating elements) and ReactDOM (rendering elements) are analogous to
  `document.createElement` and `rootElement.append()`
- `React.createElement` takes 3 arguments: type, props, children (where 3+ props
  are all children if there's multiple)
- Script tags allow you to run executables
- UNPKG lets you load any package from npm
- `ReactDOM.render()` is deprecated
- Empty props in `React.createElement` can be either `{}` or `null`
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=02%3A%20Intro%20to%20raw%20React%20APIs&em=irisjyuan%40gmail.com).
