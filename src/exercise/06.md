...
- `target` is a reference to the `<form>` DOM node
- `onSubmit=` attaches to the `<form>` not the `<input>`
- `ref=` attaches to the `<input>` not the `<form>`
- Use `event.dir` to see properties of the DOM node instead of the DOM node itself
- Use `name=` or `id=` to access through `event.target.elements`
- `htmlFor=` will tie that component to the one with that id, also allows id one to focus when the component is clicked on
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=irisjyuan%40gmail.com).
